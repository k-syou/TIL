...
def one_cution(cue_ball: Ball, target_ball: Ball, table: PoolTable, hole: tuple, other_balls:list = [Ball(0,0,0)]):
    result = []
    ...
    for d in range(4):
        ...
        r_other_balls = [ball.get_reverse_ball(table, d) for ball in other_balls] + [Ball(9, x, y) for x, y in mid_hole]
        r_hole = table.get_reverse_hole(hole, d)
        # print(f"one : cue {cue_ball.get_loc()} | target {r_target_ball.get_loc()} | hole : {r_hole}")
